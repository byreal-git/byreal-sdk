...
export function calculateTickAlignedPriceRange(info: {
  ...
  return { priceInTickLower, priceInTickUpper };
}
...
export function calculatePriceFromTick(
  ...
  return { price: price.toString(), tick: tickWithSpacing };
}

/**
 * Calculate the available price range from a TickSpacing
 */
export function calculatePriceRangeFromTickSpacing(
  tickSpacing: number,
  mintDecimalsA: number,
  mintDecimalsB: number,
) {
...
export function calculateTokenAmountsFromUsd(params: {
  ...
  const amountARaw = new BN(
    bestAmountA.mul(new Decimal(10).pow(poolInfo.mintDecimalsA)).toFixed(0),
  );
  const amountBRaw = getAmountBFromAmountA({
    priceLower,
    priceUpper,
